...
from PyQt5.QtCore import QCoreApplication, pyqtSignal, Qt, QRegularExpression, QEvent, pyqtBoundSignal, pyqtSlot
from PyQt5.QtGui import QRegularExpressionValidator, QPixmap, QValidator, QDoubleValidator
# pyqt5 docs: https://www.riverbankcomputing.com/static/Docs/PyQt4/classes.html
from PyQt5.QtWidgets import QCheckBox, QLabel, QComboBox, QSizePolicy, QInputDialog, QMainWindow, QApplication, QMessageBox, QStyleFactory, QLineEdit, QVBoxLayout, QGridLayout
...
### |___|_| |_|_|\__|_|\__,_|_|_/___\___|_|  |___/
    ...
    def __init__(self, paths, parent=None):
        ...
        # COLOR OPTION
        self.changeColor((122,0,25),(255,204,51)) # 122/0/25 = UMN MAROON,255/204/51 == UMN GOLD
    ...
    def _init_data_lists(self):
        ...
        for day in data_count:
            self.data.append(
                [None for _ in range(data_count[day])]
            )
    ...
    def _init_day1_setup(self):
        ...
        self.ui.picone4.clicked.connect(lambda: self.diagram_popup("d1_BIR hole positions.png"))
        self.ui.picone5.clicked.connect(lambda: self.diagram_popup("d1_ALF Placement.gif"))
        self.ui.barcode.clicked.connect(lambda: self.diagram_popup("barcode_placement.png"))
        self.ui.paas_attach.clicked.connect(lambda: self.diagram_popup("d2_paas_attach.png"))
        self.ui.paas_attach_2.clicked.connect(lambda: self.diagram_popup("d2_paas_attach.png"))
        self.ui.paas_attach_3.clicked.connect(lambda: self.diagram_popup("d2_paas_attach.png"))
       
    def _init_day2_setup(self):
        ## Connect
        self.ui.panelInput2.installEventFilter(self)

        self.ui.timer_instructions.clicked.connect(lambda: self.diagram_popup("d2_timer.png"))
        self.ui.picone2_2.clicked.connect(lambda: self.diagram_popup("d2_mix_epoxy.png"))
        ...
        self.day2TimerNum2  = [self.ui.hour_disp_2, self.ui.min_disp_2, self.ui.sec_disp_2]
        self.day2TimerNum3  = [self.ui.hour_disp_3, self.ui.min_disp_3, self.ui.sec_disp_3]
    ...
    def _init_day3_setup(self):
        ...
        rcItems = ['Select', 'Pass: No Continuity', 'Fail: Right Continuity', 'Fail: Left Continuity', 'Fail: Both Continuity']
        ...
        for i in range(96):
            ...
            menu1.currentIndexChanged.connect(lambda changed, index = i: checkSaveContinuity(index))
            ...
            menu2.currentIndexChanged.connect(lambda changed, index = i: checkSaveContinuity(index))
        ...
        findWidgets = lambda name : [self.ui.scrollAreaWidgetContents.findChild(QComboBox, f'{name}_{str(i).zfill(2)}') for i in range(96)]
        ...
        def checkSaveContinuity(index):
            if self.continuity[index].currentIndex() != 0 and self.wire_pos[index].currentIndex() != 0:
        ...
        for i, widgets in enumerate(zip(self.continuity,self.wire_pos)):
            for widg in widgets:
                widg.currentIndexChanged.connect(lambda changed, index = i: checkSaveContinuity(index))

        # Disable all
        self.setWidgetsDisabled(self.continuity+self.wire_pos)
        self.ui.launch_wire_tensioner.setDisabled(True)

        [combo.installEventFilter(self) for combo in self.continuity+self.wire_pos]  ## prevent value change from scroll
    ...
    def _init_pro4_setup(self):
        ...
        self.pro4Timer1 = [self.ui.hour_disp_5, self.ui.min_disp_5, self.ui.sec_disp_5] #LP
        self.pro4Timer2 = [self.ui.hour_disp_7, self.ui.min_disp_7, self.ui.sec_disp_7] #RP
        self.pro4Timer3 = [self.ui.hour_disp_11, self.ui.min_disp_11, self.ui.sec_disp_11] #LOP
        self.pro4Timer4 = [self.ui.hour_disp_13, self.ui.min_disp_13, self.ui.sec_disp_13] #ROP
        self.pro4Timers = [self.pro4Timer1, self.pro4Timer2, self.pro4Timer3, self.pro4Timer4]
    

        
        disabled_widgets = [
            self.ui.epoxyMixedLP, self.ui.epoxyMixedLOP, self.ui.epoxyMixedRP,
            self.ui.epoxyMixedROP, self.ui.epoxyInjectedLP, self.ui.epoxyInjectedRP,
            self.ui.epoxyFinishedLP, self.ui.epoxyFinishedRP,
            self.ui.epoxyAppliedLOP, self.ui.epoxyAppliedROP
            ]
        self.setWidgetsDisabled(disabled_widgets)
    ...
    def _init_pro5_setup(self):
        ...
        for i in range(96):                     #loop to fill scroll area
            ...
            hvLabel.setObjectName(f'hvLabel_{i}')   #they can't (shouldn't) all have the same name
            ...
            lCurrentEntry.setObjectName(f'lCurrent_{str(i).zfill(2)}')  # name will be lCurrent_<position>, zfill pads with zeros
            self.ui.hvGrid.addWidget(lCurrentEntry, i, 1)                # place widget in it's row, 1st column
            ...
            rCurrentEntry.setObjectName(f'rCurrent_{str(i).zfill(2)}')  # name will be rCurrent_<position>, zfill pads with zeros
            self.ui.hvGrid.addWidget(rCurrentEntry, i, 2)               # place widget in it's row, 1st column
        ...
        findLineEdit = lambda name : [self.ui.scrollContents.findChild(QLineEdit, f'{name}_{str(i).zfill(2)}') for i in range(96)]
        findCheck    = lambda name : [self.ui.scrollContents.findChild(QCheckBox, f'{name}_{i}') for i in range(96)]
        self.currentLeft  = findLineEdit('lCurrent')   # make list of currentLeft lineEdits
        self.currentRight = findLineEdit('rCurrent')   # make list of currentRight lineEdits
        self.isTripped    = findCheck('hvIsTripBool')  # make list of trip check boxes

        self.ui.scrollContents.setLayout(self.ui.hvGrid)    # add the newly created grid layout to the GUI
        ...
        def lineSaveHV(index):
            self.saveHVMeasurement(index, self.currentLeft[index].text(), self.currentRight[index].text(), self.isTripped[index].isChecked())
    ...
    def _init_timers(self):
        self.timers = [
            # Main timer
            QLCDTimer(self.ui.hour_left,    self.ui.min_left,    self.ui.sec_left,    lambda : self.Timer_0.emit(),   max_time = 28800), # 0 - Main Timer: Turns red after 8 hours
            QLCDTimer(self.ui.hour_disp_6,  self.ui.min_disp_6,  self.ui.sec_disp_6,  lambda : self.Timer_1.emit()),  # 1
            QLCDTimer(self.ui.hour_disp,    self.ui.min_disp,    self.ui.sec_disp,    lambda : self.Timer_2.emit()),  # 2
            QLCDTimer(self.ui.hour_disp_2,  self.ui.min_disp_2,  self.ui.sec_disp_2,  lambda : self.Timer_3.emit()),  # 3
            QLCDTimer(self.ui.hour_disp_3,  self.ui.min_disp_3,  self.ui.sec_disp_3,  lambda : self.Timer_4.emit()),  # 4
            QLCDTimer(self.ui.hour_disp_13, self.ui.min_disp_13, self.ui.sec_disp_13, lambda : self.Timer_5.emit()),  # 5 -> vestigial, use one that isn't associated with day 3 to keep stuff moving
            QLCDTimer(self.ui.hour_disp_8,  self.ui.min_disp_8,  self.ui.sec_disp_8,  lambda : self.Timer_6.emit()),  # 6
            QLCDTimer(self.ui.hour_disp_9,  self.ui.min_disp_9,  self.ui.sec_disp_9,  lambda : self.Timer_7.emit()),  # 7
            QLCDTimer(self.ui.hour_disp_16, self.ui.min_disp_16, self.ui.sec_disp_16, lambda : self.Timer_8.emit()),  # 8
            QLCDTimer(self.ui.hour_disp_10, self.ui.min_disp_10, self.ui.sec_disp_10, lambda : self.Timer_9.emit()),  # 9
            QLCDTimer(self.ui.hour_disp_12, self.ui.min_disp_12, self.ui.sec_disp_12, lambda : self.Timer_10.emit()),
            QLCDTimer(self.ui.hour_disp_5,  self.ui.min_disp_5,  self.ui.sec_disp_5,  lambda : self.Timer_11.emit()),
            QLCDTimer(self.ui.hour_disp_5,  self.ui.min_disp_5,  self.ui.sec_disp_5,  lambda : self.Timer_12.emit()),
            QLCDTimer(self.ui.hour_disp_7,  self.ui.min_disp_7,  self.ui.sec_disp_7,  lambda : self.Timer_13.emit()),
            QLCDTimer(self.ui.hour_disp_7,  self.ui.min_disp_7,  self.ui.sec_disp_7,  lambda : self.Timer_14.emit()),
            QLCDTimer(self.ui.hour_disp_11, self.ui.min_disp_11, self.ui.sec_disp_11, lambda : self.Timer_15.emit()),
            QLCDTimer(self.ui.hour_disp_13, self.ui.min_disp_13, self.ui.sec_disp_13, lambda : self.Timer_16.emit()),
            QLCDTimer(self.ui.hour_disp_11, self.ui.min_disp_11, self.ui.sec_disp_11, lambda : self.Timer_17.emit()),
            QLCDTimer(self.ui.hour_disp_13, self.ui.min_disp_13, self.ui.sec_disp_13, lambda : self.Timer_18.emit())
        ]
        ...
        self.running = lambda : self.mainTimer.isRunning() # Returns True if main timer is running

    def _init_failure_setup(self):
        self.failMode = [self.ui.anchorFail,self.ui.anchorFail,self.ui.strawFail,self.ui.pinFail]
        self.ui.failureComments.setDisabled(True)
        self.ui.failureComments.textChanged.connect(lambda : self.ui.failureComments.setStyleSheet(''))
        #Failures
        self.ui.failSelect.currentIndexChanged.connect(lambda : self.setFailIndex(f'{self.ui.failSelect.currentText().lower()}Fail'))
    ...
    def _init_connect(self):
        set_focus = lambda index : self.panelInput[self.day_index].setFocus() if (index == 2 and self.getCurrentPanel() == '') else None
        ...
        self.Save_Straw_Tensioner_Data.connect(lambda position, tension, uncertainty : self.DP.saveStrawTensionMeasurement(position,tension,uncertainty))
        ...
        supply_tabs = 2
        self.ui.nextButton.clicked.connect(lambda : self.ui.suppliesList.setCurrentIndex((self.ui.suppliesList.currentIndex() + 1) % supply_tabs))
        self.ui.nextButton_2.clicked.connect(lambda : self.ui.suppliesList.setCurrentIndex((self.ui.suppliesList.currentIndex() + 1) % supply_tabs))
        self.ui.previousButton.clicked.connect(lambda : self.ui.suppliesList.setCurrentIndex((self.ui.suppliesList.currentIndex() - 1) % supply_tabs))
        self.ui.previousButton_2.clicked.connect(lambda : self.ui.suppliesList.setCurrentIndex((self.ui.suppliesList.currentIndex() - 1) % supply_tabs))
    ...
    def _init_validators(self):
        ...
        validator = lambda string : QRegularExpressionValidator(QRegularExpression(string))
    ...
    def _init_widget_lists(self):
        # Start buttons
        self.startButtons = [self.ui.startbutton1, self.ui.startbutton2, self.ui.startbutton3, self.ui.startButton4, self.ui.startButton5, self.ui.startButton6, self.ui.startButton7]
        ...
        self.widgets = [
            # Day 1 Widgets
            [
                self.ui.panelInput1,    self.ui.baseInput1, self.ui.mirInput,   self.ui.birInput,
                self.ui.pirInputLA,     self.ui.pirInputLB, self.ui.pirInputLC, self.ui.pirInputRA,
                self.ui.pirInputRB,     self.ui.pirInputRC, self.ui.alfInput,   self.ui.alfInput_2,
                self.ui.leftgap,        self.ui.rightgap,   self.ui.mingap,     self.ui.maxgap,
                self.ui.epoxy_batch1,   self.ui.paasAInput, self.ui.paasCInput
            ],
            
            # Day 2 Widgets
            [
                self.ui.panelInput2,    self.ui.pallet1code,    self.ui.pallet2code,    self.ui.epoxy_batch,
                self.ui.epoxy_inject1,  self.ui.epoxy_batch_2,  self.ui.epoxy_inject2,  self.ui.temp4,
                self.ui.temp4_2,        self.ui.heat_finished,  self.ui.paasBInput
            ],
            ...
            [
                self.ui.panelInput4, self.ui.epoxy_batch_3, self.ui.epoxy_batch_4, self.ui.epoxy_batch_5,
                self.ui.epoxy_batch_6, self.ui.epoxyMixedLP, self.ui.epoxyInjectedLP, self.ui.epoxyFinishedLP,
                self.ui.epoxyMixedRP, self.ui.epoxyInjectedRP, self.ui.epoxyFinishedRP, self.ui.epoxyMixedLOP,
                self.ui.epoxyAppliedLOP, self.ui.epoxyAppliedROP, self.ui.epoxyCuredLOP, self.ui.epoxyCuredROP
            ],
            ...
            # Day 6 Widgets
            [
                self.ui.panelInput6,    self.ui.frameInput,         self.ui.mrInput1,           self.ui.mrInput2,
                self.ui.bpmirgapL,      self.ui.bpmirgapR,          self.ui.epoxy_batch41,      self.ui.epoxy_applied41,
                self.ui.epoxy_batch42,  self.ui.epoxy_batch42_2,    self.ui.epoxy_applied42,    self.ui.temp4_4,
                self.ui.temp4_5,        self.ui.heat_finished4 
            ],

            # Day 7 Widgets
            [
                self.ui.panelInput7, self.ui.epoxy_batch5_2, self.ui.epoxy_applied5_2, self.ui.epoxy_batch5_3, self.ui.epoxy_applied5_3 
            ]

        ]

    def _init_panel_input(self):
        self.panelInput =   [self.ui.panelInput1, self.ui.panelInput2, self.ui.panelInput3, self.ui.panelInput4, self.ui.panelInput5, self.ui.panelInput6, self.ui.panelInput7]
    ...
    def _init_finish_button(self):
        ...
        self.finishButton.clicked.connect(
            lambda : self.timers[5].stop() if self.stepsList.allStepsChecked() and self.day == 3 else None
        )   # Stop the day 3 timer when finish button is pushed     TODO: for pro 5?
        self.finishButton.clicked.connect(lambda clicked : self.stopRunning(self.finishButton.text() == 'Pause'))
    ...
    def _init_highlighting(self):
        ...
        self.ui.failSelect.highlighted.connect(lambda index: self.ui.failSelect.setStyleSheet(''))
        self.ui.anchorFail.highlighted.connect(lambda : self.ui.anchorFail.setStyleSheet(''))
        self.ui.pinFail.highlighted.connect(lambda : self.ui.pinFail.setStyleSheet(''))
        self.ui.strawFail.highlighted.connect(lambda : self.ui.strawFail.setStyleSheet(''))
        self.ui.positionSelect.highlighted.connect(lambda : self.ui.positionSelect.setStyleSheet(''))
...
###                                                
    ...
    def validateInput(self, widgets = None, indices = None):
        ...
        passFailDict = {
            'QLineEdit' : lambda widget : widget.validator().validate(widget.text(), 0)[0] == QValidator.Acceptable,
            'QComboBox' : lambda widget : not (widget.currentIndex() == 0),
            'list'      : lambda l      : self.validateInput(widgets = l)
        }

        errorStyleSheet = 'QLineEdit { background-color:rgb(149, 186, 255); }'\
    ...
    def checkDevice(self, type):
        ...
        if len(arduino_ports)<1:
            error = True
            self.generateBox('critical', 'Tension device not found', 'Plug tension box into any USB port and try again.' )
        
        if len(arduino_ports) > 1:
            error = True
            self.generateBox('critical', 'More than one Arduino found', 'Disconnect the ones not in use and try again.')
        return error
...
###       |_|                                                                                       |_|    
    ...
    def openGUI(self, btn):
        ...
        # Steps List
        self.stepsList = StepList(self.ui.stepsBox, self.DP.loadSteps(), self.checkProgress, self.diagram_popup)
    ...
    def closeEvent(self, event):
        ...
        if self.running():
            ...
            self.generateBox('critical', 'Exit Warning', 'GUI cannot be exited while running.')
            event.ignore()
        else:
            #reply = QMessageBox.warning(self, 'Exit Warning', 'Are you sure you want to exit?', QMessageBox.Yes, QMessageBox.No)
            reply = self.generateBox('warning', 'Exit Warning', 'Are you sure you want to exit?', True)
...
### |_|  |_|_|___/\___/_/   \___/|_| |_|___/\___/|_|   \__\___|\__,_|
    ...
    def eventFilter(self, obj, event):  # obj = QWidget, event = QEvent
        if isinstance(obj, QComboBox):  # if it's a combo box...
            ...
            if event.type() == QEvent.KeyPress:     # if a key was pressed...
                if event.key() in (Qt.Key_Return, Qt.Key_Enter):    # if that key was return/enter...
                    event.ignore()                                      # ignore it!
    ...
    def backToDaySelect(self):
        ...
        [self.resetDay1, self.resetDay2, self.resetDay3, self.resetPro4, self.resetPro5, self.resetDay6, self.resetDay7][self.day_index]()
        ...
        self.suppliesList = SuppliesList(self.ui.Tools, self.ui.Parts, self.ui.Supplies,self.ui.MoldRelease)
        self.suppliesList.setDiagramPopup(self.diagram_popup)
        self.ui.suppliesList.setCurrentIndex(0)
    ...
    '''
    def partsError(self):
        self.generateBox('critical', 'Parts List Not Checked', 'All tools, parts, and supplies must be checked off before day can begin.')
    ...
    def moldReleaseError(self):
        ...
        self.generateBox('critical', 'Mold Release List Not Checked', f'The following items need to be mold released prior to starting this day:\n{str_items}')
        self.ui.tabWidget.setCurrentIndex(1)
    ...
    def resizeGUI(self):
        ...
        if resized:
            self.generateBox('warning', 'Screen Size Warning', f'The current screen size ({w} x {h}) is too small for the optimal GUI size ({wOpt} x {hOpt}). The GUI has been resized, but UI elements may not display correctly.')
    ...
    def setFailIndex(self, name):
        ...
        if self.ui.failSelect.currentIndex() == 0 or self.ui.failSelect.currentText() == 'Other':
    ...
    def enableAdditionalFailure(self):
        ...
        if self.ui.failSelect.currentText() != 'Select failed item' and self.ui.failSelect.currentText() != 'Other':
    ...
    def failure(self):
        ...
        if self.ui.failSelect.currentText() == 'Other' and self.ui.failureComments.toPlainText() == '':
        ...
        position = int(position_label.currentText()) if position_label is not None else None
        ...
        failure_mode = failure_mode_label.currentText() if failure_mode_label is not None else None
    ...
    def Change_worker_ID(self, btn):
        ...
        if label == 'Log In':
            ...
            if Current_worker == 'GOLDY' or Current_worker == 'WK-DAMBROSE01':
                self.changeColor((122,0,25),(255,204,51)) #122/0/25 = maroon, 255/204/51 = gold
                self.ui.scrollArea.setStyleSheet("background-color: rgb(122, 0, 25);")
                self.ui.scrollAreaHV.setStyleSheet("background-color: rgb(122, 0, 25);")
            elif Current_worker == 'DARK MODE' or Current_worker == 'WK-AARNETT01' or Current_worker == 'WK-KBOEDIGH01':  # enter dark mode to change to dark mode, Adam and Kaitlin like it too
                self.changeColor((26,26,26),(255,255,255)) # 26/26/26 = 10% brightness, 255/255/255 = white
            ...
            elif Current_worker == 'SURPRISE ME':   # enter surprise me to get a random color
                ...
                self.ui.scrollArea.setStyleSheet(f"background-color: rgb({backR}, {backG}, {backB});")
                self.ui.scrollAreaHV.setStyleSheet(f"background-color: rgb({backR}, {backG}, {backB});")
            elif Current_worker == 'BURN MY CORNEAS':   # enter burn my corneas to get back to normal colors
                self.changeColor((255,255,255),(0,0,0)) # 255/255/255 = white back, 0/0/0 = black text
                self.ui.scrollArea.setStyleSheet("background-color: rgb(255, 255, 255);")
                self.ui.scrollAreaHV.setStyleSheet("background-color: rgb(255, 255, 255);")
            ...
            if ok and Current_worker != '':
                if not self.DP.validateWorkerID(Current_worker):
                    ...
                    self.generateBox('critical', 'Login Error', 'This worker ID is already logged in.')
                else:
...
### |____/ \__,_| \_/ \___| |____/ \__,_|\__\__,_| |_|  |_|\___|\__|_| |_|\___/ \__,_|___/
    ...
    def saveContinuityMeasurement(self,position,continuity_str,wire_pos_str):
        ...
        if self.continuity[position].currentText() != continuity_str or self.wire_pos[position].currentText() != wire_pos_str:
    ...
    def saveComments(self, comments = ''):
        ...
        if comments == '':
            ...
            box = [self.ui.commentBox1, self.ui.commentBox2, self.ui.commentBox3, self.ui.commentBox_2, self.ui.day5CommentBox, self.ui.commentBox4, self.ui.commentBox5][self.day_index]
            # Extract text
        ...
        try:
            self.DP.saveComment(comments, self.getCurrentPanel(), self.day)   # try to save comment
        except Exception:
            # If it fails, generate a message box and return
            self.generateBox('critical', 'Save Error', 'Error encountered trying to save comments.')
            return
...
###       |_|                                                                                       
    ...
    def updateData(self):
        ...
        [self.updateDataProcess1, self.updateDataProcess2, self.updateDataProcess3, self.updateDataProcess4,  self.updateDataProcess5, self.updateDataProcess6, self.updateDataProcess7][self.day_index]()
        
        # Process the updated data list by replacing all elements in the list that don't pass bool(el) with None
        self.data[self.day_index] = [(el if bool(el) else None) for el in self.data[self.day_index]]
    ...
    def updateDataProcess1(self):
        ...
        self.data[self.day_index][12] = int(self.ui.leftgap.currentText()) if self.ui.leftgap.currentIndex() != 0 else None
        self.data[self.day_index][13] = int(self.ui.rightgap.currentText()) if self.ui.rightgap.currentIndex() != 0 else None
        self.data[self.day_index][14] = int(self.ui.mingap.currentText()) if self.ui.mingap.currentIndex() != 0 else None
        self.data[self.day_index][15] = int(self.ui.maxgap.currentText()) if self.ui.maxgap.currentIndex() != 0 else None
    ...
    def updateDataProcess2(self):
        ...
        self.data[self.day_index][4] = self.timerTuple(self.timers[2])  # Lower Epoxy Time
        self.data[self.day_index][5] = self.ui.epoxy_batch_2.text()     # Upper Epoxy
        self.data[self.day_index][6] = self.timerTuple(self.timers[3])  # Upper Epoxy Time
        self.data[self.day_index][7] = float(self.ui.temp4.text())     if self.ui.temp4.text()   else None # PAAS-A Max Temp
        self.data[self.day_index][8] = float(self.ui.temp4_2.text())   if self.ui.temp4_2.text() else None # PAAS-B Max Temp
    ...
    def updateDataProcess4(self):
        ...
        self.data[self.day_index][1]  = self.ui.epoxy_batch_3.text()      # clear epoxy left - batch
        self.data[self.day_index][2]  = self.timerTuple(self.timers[11])  # clear epoxy left - application duration
        self.data[self.day_index][3]  = self.timerTuple(self.timers[12])  # clear epoxy left - cure duration
        self.data[self.day_index][4]  = self.ui.epoxy_batch_4.text()      # clear epoxy right - batch
        self.data[self.day_index][5]  = self.timerTuple(self.timers[13])  # clear epoxy right - application duration
        self.data[self.day_index][6]  = self.timerTuple(self.timers[14])  # clear epoxy right - cure duration
        self.data[self.day_index][7]  = self.ui.epoxy_batch_5.text()      # silver epoxy left - batch
        self.data[self.day_index][8]  = self.timerTuple(self.timers[15])  # silver epoxy left - application duration
        self.data[self.day_index][9]  = self.timerTuple(self.timers[17])  # silver epoxy left - cure duration
        self.data[self.day_index][10] = self.ui.epoxy_batch_6.text()      # silver epoxy right - batch
        self.data[self.day_index][11] = self.timerTuple(self.timers[16])  # silver epoxy right - application duration
        self.data[self.day_index][12] = self.timerTuple(self.timers[18])  # silver epoxy right - cure duration
    ...
    def updateDataProcess6(self):
        ...
        self.data[self.day_index][4]  = int(self.ui.bpmirgapL.currentText()) if self.ui.bpmirgapL.currentIndex() != 0 else None
        self.data[self.day_index][5]  = int(self.ui.bpmirgapR.currentText()) if self.ui.bpmirgapR.currentIndex() != 0 else None
        ...
        self.data[self.day_index][11] = float(self.ui.temp4_4.text()) if self.ui.temp4_4.text() else None
        self.data[self.day_index][12] = float(self.ui.temp4_5.text()) if self.ui.temp4_5.text() else None
...
### |_____\___/ \__,_|\__,_| |____/ \__,_|\__\__,_| |_|  |_|\___|\__|_| |_|\___/ \__,_|___/
    ...
    def loadDay(self):
        ...
        data, elapsed_time, steps_completed = self.DP.loadData() #data should be renamed so there isn't self.data and data
        ## If no new data is found, return early
        if not any(el is not None for el in data[1:]): # Everything in data list except for panel
            if not self.day == 5:
                return
        ...
        parse_day = [self.parseDay1Data, self.parseDay2Data, self.parseDay3Data, self.parsePro4Data, self.parsePro5Data, self.parseDay6Data, self.parseDay7Data]
        ...
        self.setWidgetsEnabled(
            [
                self.ui.saveButtons.buttons()[self.day_index],
                self.finishButton
            ]
        )
        ...
        self.setWidgetsDisabled(
            [
                self.startButtons[self.day_index],
                self.ui.dayReturnButton
            ]
        )
...
###                                                              |_|            |___/ 
    ...
    def displayAllHVMeasurements(self, data):
        for index, measurements in enumerate(data):
            ...
            if any(current is not None for current in measurements):
                self.displayHVMeasurement(index, current_left, current_right, is_tripped)
    ...
    def parseDay2Data(self, data):
        ...
        if data[0] is not None:                     # if panel num data exists
            self.ui.panelInput2.setText(data[0])        # set text of panel line edit widget to number
            self.ui.panelInput2.setDisabled(True)       # disable panel line edit

        # pallet codes----------------------------------------------------------------------------
        if data[1] is not None:                     # if upper pallet code data exists
            self.ui.pallet1code.setText(data[1])        # set text of upper pallet line edit widget to code
        ...
        if data[2] is not None:                     # if lower pallet code data exists
            self.ui.pallet2code.setText(data[2])        # set text of lower pallet line edit widget to code
        ...
        if data[1] is not None and data[2] is not None:     # if BOTH pallet codes exist
            self.ui.epoxy_batch.setEnabled(True)                # enable lower epoxy batch line edit widget
            self.ui.epoxy_mixed.setEnabled(True)                # enable lower epoxy mix button
            self.ui.epoxy_batch_2.setEnabled(True)              # enable upper epoxy batch line edit widget
            self.ui.epoxy_mixed_2.setEnabled(True)              # enable upper epoxy mix button
            self.ui.launch_straw_tensioner.setEnabled(True)     # enable launch straw tensioner button

        # lower epoxy ----------------------------------------------------------------------------
        if data[3] is not None:                     # if lower epoxy batch num data exists
            self.ui.epoxy_batch.setText(data[3])        # set text of lower epoxy line edit widget to num

        if data[4] is not None:                     # if lower epoxy timer data exists (if timer has been started)
            self.ui.epoxy_mixed.setDisabled(True)       # disable lower epoxy mixed button
            self.ui.epoxy_batch.setDisabled(True)       # disable lower epoxy batch line edit widget

            elapsed_time, running = data[4]             # extract data from timer tuple [<timedeltaObj>, <isRunningBool>]
            self.timers[2].setElapsedTime(elapsed_time) # set time on corresponding timer
            if running:                                 # if the the loaded data indicates the timer should be running
                self.startTimer(2)                          # start the timer
                self.ui.epoxy_inject1.setEnabled(True)      # enable lower epoxy inject button (button that stops the timer)
            else:                                       # the loaded data indicates the timer should NOT be running
                self.ui.epoxy_inject1.setDisabled(True)     # disable lower epoxy inject button (timer must have been stopped)

        # upper epoxy ----------------------------------------------------------------------------
        if data[5] is not None:                     # if upper epoxy batch num data exists
            self.ui.epoxy_batch_2.setText(data[5])      # set text of upper epoxy line edit widget to num

        if data[6] is not None:                     # if upper epoxy timer data exists (if timer has been started)
            self.ui.epoxy_mixed_2.setDisabled(True)     # disable upper epoxy mixed button
            self.ui.epoxy_batch_2.setDisabled(True)     # disable upper epoxy batch line edit widget

            elapsed_time, running = data[6]             # extract data from timer tuple [<timedeltaObj>, <isRunningBool>]
            self.timers[3].setElapsedTime(elapsed_time) # set time on corresponding timer
            if running:                                 # if loaded data indicates the timer should be running
                self.startTimer(3)                          # start the timer
                self.ui.epoxy_inject2.setEnabled(True)      # enable upper epoxy inject button (button that stops the timer)
            else:                                       # the loaded data indicates the timer should NOT be running
                self.ui.epoxy_inject2.setDisabled(True)     # disable upper epoxy inject button (timer must have been stopped)
                self.ui.heat_start.setEnabled(True)         # enable heat start button (epoxy stuff has been finished)

        # heat stuff -----------------------------------------------------------------------------
        if data[7] is not None:                     # if paas A max temp num data exists
            self.ui.temp4.setText(str(data[7]))         # set text of paas A max temp line edit widget to num
        if data[8] is not None:                     # if paas B max temp num data exists
            self.ui.temp4_2.setText(str(data[8]))       # set text of paas B max temp line edit widget to num
        ...
        if data[9] is not None:                     # if heat timer data exists (if timer has been started)
            ...
            elapsed_time, running = data[9]             # extract timer tuple data [<timedeltaObj>, <isRunningBool>]
            self.timers[4].setElapsedTime(elapsed_time) # set time on corresponding timer
            if running:                                 # if loaded data indicates the timer should be running
                self.startTimer(4)                          # start the timer
                self.ui.heat_finished.setEnabled(True)      # enable heat finish button (button that stops the timer)
                self.ui.temp4.setEnabled(True)              # enable paas A max temp line edit widget
                self.ui.temp4_2.setEnabled(True)            # enable paas B max temp line edit widget
            else:                                       # loaded data indicates the timer should NOT be running
                self.ui.heat_finished.setDisabled(True)     # disable heat finished button (timer must've been stopped)
                self.ui.temp4.setDisabled(True)             # disable paas A max temp line edit widget
                self.ui.temp4_2.setDisabled(True)           # disable paas B max temp line edit widget
    ...
    def parsePro4Data(self,data):
        ...
        if data[2] is not None:                         # if timer data for left pin application time exists
            self.ui.epoxyMixedLP.setDisabled(True)          # the timer has been started so disable mixed button
            self.ui.epoxy_batch_3.setDisabled(True)         # and epoxy batch data has been entered so disable batch input widget
            self.ui.epoxy_batch_3.setText(data[1])          # the batch ID data must've been entered so set the batch input widget to data[1]

            elapsed_time, running = data[2]                 # data[2] is a timer tuple, (timedelta>, <bool>)
            self.timers[11].setElapsedTime(elapsed_time)    # timer 11 corresponds to the left pin application duration
            if running:                                     # if the timer is running
                self.startTimer(11)                             # resume running timer 11
                self.ui.epoxyInjectedLP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyInjectedLP.setDisabled(True)       # disable the button that stops it since it has already finished
            
            # if timer data for application timer exists, then data for cure time also exists, since they're started by the same button
            elapsed_time, running = data[3]                 # data [3] is a timer tuple, (<timedelta>, <bool>)
            self.timers[12].setElapsedTime(elapsed_time)    # timer 12 corresponds to the left pin cure duration
            if running:                                     # if the timer is running
                self.startTimer(12)                             # resume running timer 12
                self.ui.epoxyFinishedLP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyFinishedLP.setDisabled(True)       # disable the button that stops it since it has already finished
        elif data[1] is not None:                       # ELSE if batch ID data for left pin exists (batch entered, not started)
            self.ui.epoxy_batch_3.setText(data[1])          # set the batch input widget to data[1]
        ...
        if data[5] is not None:                         # if timer data for right pin application time exists
            self.ui.epoxyMixedRP.setDisabled(True)          # the timer has been started so disable mixed button
            self.ui.epoxy_batch_4.setDisabled(True)         # and epoxy batch data has been entered so disable batch input widget
            self.ui.epoxy_batch_4.setText(data[4])          # the batch ID data must've been entered so set the batch input widget to data[4]

            elapsed_time, running = data[5]                 # data[5] is a timer tuple, (timedelta>, <bool>)
            self.timers[13].setElapsedTime(elapsed_time)    # timer 13 corresponds to the right pin application duration
            if running:                                     # if the timer is running
                self.startTimer(13)                             # resume running timer 13
                self.ui.epoxyInjectedRP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyInjectedRP.setDisabled(True)       # disable the button that stops it since it has already finished
            
            # if timer data for application timer exists, then data for cure time also exists, since they're started by the same button
            elapsed_time, running = data[6]                 # data [6] is a timer tuple, (<timedelta>, <bool>)
            self.timers[14].setElapsedTime(elapsed_time)    # timer 14 corresponds to the right pin cure duration
            if running:                                     # if the timer is running
                self.startTimer(14)                             # resume running timer 14
                self.ui.epoxyFinishedRP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyFinishedRP.setDisabled(True)       # disable the button that stops it since it has already finished
        elif data[4] is not None:                       # ELSE if batch ID data for left pin exists (batch entered, not started)
            self.ui.epoxy_batch_4.setText(data[4])          # set the batch input widget to data[4]
        ...
        if data[8] is not None:                         # if timer data for left omega application time exists
            self.ui.epoxyMixedLOP.setDisabled(True)         # the timer has been started so disable mixed button
            self.ui.epoxy_batch_5.setDisabled(True)         # and epoxy batch data has been entered so disable batch input widget
            self.ui.epoxy_batch_5.setText(data[7])          # the batch ID data must've been entered so set the batch input widget to data[7]

            elapsed_time, running = data[8]                 # data[8] is a timer tuple, (timedelta>, <bool>)
            self.timers[15].setElapsedTime(elapsed_time)    # timer 15 corresponds to the left omega application duration
            if running:                                     # if the timer is running
                self.startTimer(15)                             # resume running timer 15
                self.ui.epoxyAppliedLOP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyAppliedLOP.setDisabled(True)       # disable the button that stops it since it has already finished
            
            # if timer data for application timer exists, then data for cure time also exists, since they're started by the same button
            elapsed_time, running = data[9]                 # data [9] is a timer tuple, (<timedelta>, <bool>)
            self.timers[16].setElapsedTime(elapsed_time)    # timer 16 corresponds to the left omega cure duration
            if running:                                     # if the timer is running
                self.startTimer(16)                             # resume running timer 16
                self.ui.epoxyCuredLOP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyCuredLOP.setDisabled(True)       #disable the button that stops it since it has already finished
        elif data[7] is not None:                       # ELSE if batch ID data for left pin exists (batch entered, not started)
            self.ui.epoxy_batch_5.setText(data[1])          # set the batch input widget to data[7]
        ...
        # RIGHT OMEGA
        if data[11] is not None:                        # if timer data for right omega application time exists
            self.ui.epoxyMixedROP.setDisabled(True)         # the timer has been started so disable mixed button
            self.ui.epoxy_batch_6.setDisabled(True)         # and epoxy batch data has been entered so disable batch input widget
            self.ui.epoxy_batch_6.setText(data[10])         # the batch ID data must've been entered so set the batch input widget to data[10]

            elapsed_time, running = data[11]                 # data[11] is a timer tuple, (timedelta>, <bool>)
            self.timers[17].setElapsedTime(elapsed_time)    # timer 17 corresponds to the right omega application duration
            if running:                                     # if the timer is running
                self.startTimer(17)                             # resume running timer 15
                self.ui.epoxyAppliedROP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyAppliedROP.setDisabled(True)       # disable the button that stops it since it has already finished
            
            # if timer data for application timer exists, then data for cure time also exists, since they're started by the same button
            elapsed_time, running = data[12]                 # data [12] is a timer tuple, (<timedelta>, <bool>)
            self.timers[18].setElapsedTime(elapsed_time)    # timer 18 corresponds to the right omega cure duration
            if running:                                     # if the timer is running
                self.startTimer(18)                             # resume running timer 18
                self.ui.epoxyCuredROP.setEnabled(True)        # and enable the button that stops it
            else:                                           # if it's not running, it must be finished since there is recorded time for it
                self.ui.epoxyCuredROP.setDisabled(True)       #disable the button that stops it since it has already finished
        elif data[10] is not None:                       # ELSE if batch ID data for left pin exists (batch entered, not started)
            self.ui.epoxy_batch_6.setText(data[10])          # set the batch input widget to data[10]
    ...
    def parseDay6Data(self, data):
        ...
        if data[7] is not None:
            ...
            try:
                ...
                elapsed_time, running = data[7]
                self.generateBox('critical', 'Data Processor Error', 'SQL Processor inactive, BP/IR epoxy timer may not function.')
        ...
        if data[10] is not None:
            ...
            try:
                ...
                elapsed_time, running = data[10]
                self.generateBox('critical', 'Data Processor Error', 'SQL Processor inactive, Frame epoxy timer may not function.')
        ...
        if data[13] is not None:
            ...
            try:
                ...
                elapsed_time, running = data[13]
                self.generateBox('critical', 'Data Processor Error', 'SQL Processor inactive, heating timer may not function.')
...
###              |___/                    
    ...
    def day1part1(self):
        ...
        self.setWidgetsDisabled(
            [
                ...
                self.ui.startbutton1,self.ui.panelInput1, 
                # All parts input
                self.ui.baseInput1, self.ui.birInput, self.ui.pirInputLA, self.ui.pirInputLB, 
                self.ui.pirInputLC, self.ui.pirInputRA, self.ui.pirInputRB, self.ui.pirInputRC,
                self.ui.mirInput, self.ui.alfInput, self.ui.alfInput_2, self.ui.paasAInput,
                self.ui.paasCInput
            ]
        )
        self.setWidgetsEnabled(
            [
                ...
                self.ui.epoxy_batch1, self.ui.epoxy_mixed1
            ]
        )
    ...
    def resetDay1(self):
        ...
        input_widgets = [
            self.ui.panelInput1, self.ui.baseInput1, self.ui.birInput, self.ui.pirInputLA,
            self.ui.pirInputLB, self.ui.pirInputLC, self.ui.pirInputRA, self.ui.pirInputRB,
            self.ui.pirInputRC, self.ui.mirInput, self.ui.alfInput, self.ui.epoxy_batch1,
            self.ui.paasAInput, self.ui.paasCInput
        ]
...
###              |___/                       
    ...
    def day2part1(self):
        ...
        self.setWidgetsDisabled(
            [self.ui.startbutton2, self.ui.panelInput2, self.ui.pallet1code, self.ui.pallet2code]
        )
    ...
    def day2part2(self):
        ...
        if (self.timers[2].getElapsedTime == 1800):
            self.generateBox('critical', 'Epoxy Timer at 30 minutes', 'Rotate straws into alignment.')
        if (self.timers[2].getElapsedTime == 3600):
            self.generateBox('critical', 'Epoxy Timer at 1 hour', 'Rotate straws into alignment.')
        if (self.timers[2].getElapsedTime == 5400):
            self.generateBox('critical', 'Epoxy Timer at 1 hour 30 minutes', 'Rotate straws into alignment.')                      
    ...
    def day2part2_4(self):
        ...
        self.setWidgetsDisabled(
            [self.ui.heat_finished, self.ui.temp4, self.ui.temp4_2]
        )
...
###                                          
    ...
    def pro4part0(self):
        ...
        self.setWidgetsEnabled(
            [self.ui.panelInput4, self.ui.epoxy_batch_3, self.ui.epoxy_batch_4,
            self.ui.epoxy_batch_5, self.ui.epoxy_batch_6, self.ui.epoxyMixedLP,
            self.ui.epoxyMixedRP]
        )

        self.setWidgetsDisabled(
            [self.ui.epoxyInjectedLP, self.ui.epoxyInjectedRP,
            self.ui.epoxyFinishedLP, self.ui.epoxyFinishedRP, self.ui.epoxyAppliedLOP,
            self.ui.epoxyAppliedROP, self.ui.startButton4, self.ui.epoxyCuredLOP,
...
###               |___/                
    ...
    def day6part1(self):
        ...
        self.setWidgetsEnabled(
            [
                self.ui.startButton6,   self.ui.panelInput4,    self.ui.frameInput,
                self.ui.mrInput1,       self.ui.mrInput2
            ]
        )
        ...
        self.setWidgetsEnabled(
            [
                self.ui.epoxy_batch41,  self.ui.epoxy_mixed41,
                self.ui.bpmirgapL,      self.ui.bpmirgapR
            ]
        )
    ...
    def day6part2(self):
        ...
        self.setWidgetsDisabled(
            [
                self.ui.bpmirgapL, self.ui.bpmirgapR, self.ui.epoxy_batch41, self.ui.epoxy_mixed41
            ]
        )
...
###               |___/                              
    ...
    def day7part1(self):
        ...
        self.setWidgetsEnabled(
            [
                self.ui.epoxy_mixed5_2, self.ui.epoxy_batch5_2,
                self.ui.epoxy_mixed5_3, self.ui.epoxy_batch5_3
            ]
        )
...
###                                                                                       
    ...
    def generateBox(self, type, title, text, question = False):
        iconDict = {'question' : QMessageBox.Question, 'warning' : QMessageBox.Warning, 'critical' : QMessageBox.Critical}
        box = QMessageBox()
    ...
    def diagram_popup(self, diagram):
        ...
        if not w == 0 and not h == 0:
            if w > wOpt or h > hOpt:
                self._diagram.setPixmap(pixmap.scaled(wOpt - 20, hOpt - 20, Qt.KeepAspectRatio))
    ...
    def strawTensionPopup(self):
        ...
        if self.strawTensionWindow is None:                             # if there's no strawTension window present...
            self.strawTensionWindow = StrawTension(                         # make one! (creating it doesn't show it though)
                saveMethod = lambda position, tension, uncertainty :            # pass it a save method, so it can...
                    saveStrawTensionMeasurement(position, tension, uncertainty),    # ...save data to the DB
                inGUI= (self.ui.panelInput2.text()[2:])     # let the tensioner gui know what panel it's being used for
            )
        self.strawTensionWindow.show()                                  # show the window
        self.strawTensionWindow.resize(1600,1200)                       # resize for readability (default is 400x200?)
        ...
        if self.checkDevice('tensionBox') == False:
            if self.tensionBoxWindow is None:
                self.tensionBoxWindow = TensionBox(
                    saveMethod = (
                        lambda panel, is_straw, position, length, frequency, pulse_width, tension :
                        self.DP.saveTensionboxMeasurement(self.getCurrentPanel(), is_straw,position,length,frequency,pulse_width,tension)
                    ),
...
def checkPackages():
    packageList = [     # list of packages to check, each tuple has the name of the package and a boolean to determine if the version is correct
        ...
        ('PyQt5',PYQT_VERSION_STR=='5.12.2'),           # PyQt5 doesn't have a __version__ member, but PyQt5.Qt has PYQT_VERSION_STR
    ...
    for package in packageList:
        if not package[1]:  # if the boolean statement is false (if the version is incorrect)
...
if __name__=="__main__":
    sys.excepthook = except_hook    # catch annoying "python has stopped working" error on crash
