...
class Silver(QMainWindow):
    ...
    def initialData(self):
        ...
        if not len(self.palletNum) == 8 or not self.palletNum.startswith('CPAL') or not self.palletNum[4:].isnumeric():
            valid[1] = False

            if self.palletNum == '':
                self.ui.palletNumInput.setStyleSheet('background-color:rgb(149, 186, 255)')
            else:
        ...
        if not len(self.epoxyBatch) == 12 or not self.epoxyBatch.startswith('SE.') or not int(self.epoxyBatch[3:5]) in range(1,13)\
             or not int(self.epoxyBatch[5:7]) in range(1,32) or not int(self.epoxyBatch[7:9]) in range(17,(datetime.datetime.now().year - 2000) + 1)\
        ...
        while not passed:
            try:
                ...
                reply = QMessageBox.critical(self, "Testing Error", "Unable to test this pallet:\n" + error.message + "\n\nRemove failed straws?", QMessageBox.Yes, QMessageBox.No)
    ...
    def saveData(self):
        if os.path.exists(self.palletDirectory + self.palletID + '\\' + self.palletNum + '.csv'):
            ...
            with open(self.palletDirectory + self.palletID + '\\' + self.palletNum + '.csv', 'a') as palletWrite:
                palletWrite.write('\n')
                palletWrite.write(datetime.datetime.now().strftime('%Y-%m-%d_%H:%M') + ',silv,')
        ...
        reply = QMessageBox.question(self, "Failed Straws", "Were all the insertions successful?", QMessageBox.Yes, QMessageBox.No)

        if reply == QMessageBox.Yes:
            self.uploadData()
            self.resetGUI()
        else:
            QMessageBox.question(self, 'Pallet cleaned?', "Please remove the failed straws.\n The edit pallet menu will pop up next.", QMessageBox.Ok)
            self.editPallet()
            self.uploadData()
            self.resetGUI()
