...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.byHand_button = QtWidgets.QPushButton(
            self.resistance_tab
        )  # calls measureByHand() in main.py
        self.collect_button = QtWidgets.QPushButton(
            self.resistance_tab
        )  # calls collectData() in main.py
        self.reset_button = QtWidgets.QPushButton(
            self.resistance_tab
        )  # calls reset() in main.py
        self.save_button = QtWidgets.QPushButton(
            self.resistance_tab
        )  # calls saveReset() in main.py
        self.editPallet_button = QtWidgets.QPushButton(
            self.resistance_tab
        )  # calls editPallet() in main.py

        # Set Geometry
        self.collect_button.setGeometry(QtCore.QRect(40, 20, 180, 80))
        self.byHand_button.setGeometry(QtCore.QRect(260, 20, 180, 80))
        self.reset_button.setGeometry(QtCore.QRect(700, 20, 180, 80))
        self.save_button.setGeometry(QtCore.QRect(920, 20, 180, 80))
        self.editPallet_button.setGeometry(QtCore.QRect(480, 20, 180, 80))
        ...
        self.editPallet_button.setFont(button_font)
        ...
        self.editPallet_button.setAutoDefault(False)
        ...
        # QObject Lists
        self.boxlist = [
            QtWidgets.QGroupBox(self.resistance_tab) for n in range(24)
        ]  # list of QGroupBox objects
        self.strawID_label_list = []  # list of QLabel objects
        self.meas_label_list = [[] for n in range(24)]  # list of QLabel objects
        self.meas_led_list = [
            [] for n in range(24)
        ]  # list of QLabel objects for color indicators
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.tab_widget.setTabText(
            self.tab_widget.indexOf(self.worker_portal_tab),
            _translate("MainWindow", "Worker Portal"),
        )
        self.tab_widget.setTabText(
            self.tab_widget.indexOf(self.resistance_tab),
            _translate("MainWindow", "Resistance Test"),
        )
        ...
        self.editPallet_button.setText(_translate("MainWindow", "View/Edit Pallet"))

        for n in range(0, 24):
            self.boxlist[n].setTitle(_translate("MainWindow", "Position " + str(n)))
            self.strawID_label_list[n].setText(_translate("MainWindow", "st####"))
            for i in range(0, 4):
                self.meas_label_list[n][i].setText(
                    _translate("MainWindow", RES_MEAS[i] + ":")
                )
                self.meas_input_list[n][i].setPlaceholderText(
                    _translate("MainWindow", RES_MEAS[i])
                )
