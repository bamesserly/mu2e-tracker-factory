...
# Navigates to ('Straw lab GUIs/Remove'), imports removeStraw.py
sys.path.insert(0, '\\\\MU2E-CART1\\Users\\Public\\Database Backup')
...
class CompletionTrack(QtWidgets.QDialog):
    ...
    def interpretEditPallet(self, CPAL, lastTask, straws, passfail):
        ...
        print("remove_straws: " + str(remove_straws))
    ...
    def configureSaveData(self):
        ...
        for pos in range(24):
            for i in range(4):
                resistance = self.meas_input[pos][i].text()     #Measurement for respective position and index
            ...
            for pallet in os.listdir(self.palletDirectory + palletid + '\\'):
                if self.palletNumber + '.csv' == pallet:
                    with open(self.palletDirectory + palletid + '\\' + pallet, 'a') as file:
                        # Record Session Data
                        file.write("\n"+datetime.now().strftime('%Y-%m-%d_%H:%M') + ',') # Date
                        file.write('ohms,') # Test ID
    ...
    def main(self):
        while self.run_program:
            while not self.credentials:
                ...
                time.sleep(.01)
                app.processEvents()
            while not self.calledInitializePallet:
                self.initializePallet()
            while self.calledInitializePallet and self.credentials and self.run_program:
                self.checkCredentials()
                self.lockGUI()
                time.sleep(.01)
                app.processEvents()
