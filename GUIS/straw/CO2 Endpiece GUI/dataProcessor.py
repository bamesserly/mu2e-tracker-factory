...
sys.path.insert(0, str(Path(Path(__file__).resolve().parent.parent.parent.parent / 'Database')))
from databaseClassesStraw import (
    Comment, Procedure, Station, Worker, DM, StrawPrep, Resistance,CO2,
    LeakTest, LaserCut, SilverEpoxy,StrawPresent, StrawPosition, StrawLocationType,
    CuttingPallet, LoadingPallet, Pallet, StrawLocation, Straw
)
...
class TxtDataProcessor(DataProcessor):
    ...
    def saveData(self):
        if os.path.exists(self.palletDirectory + self.palletID + '\\' + self.palletNum + '.csv'):
            ...
            with open(self.palletDirectory + self.palletID + '\\' + self.palletNum + '.csv', 'a') as palletWrite:
                ...
                for straw in self.gui.strawList:
        ...
        with open(self.epoxyDirectory + self.palletNum + '.csv', 'w+') as file:
            ...
            file.write(self.palletID + ',' + self.gui.epoxyBatch + ',' + self.gui.DP190Batch + ',')
            file.write(str(self.gui.ui.hour_disp.intValue()) + ':' + str(self.gui.ui.min_disp.intValue()) + ':' + str(self.gui.ui.sec_disp.intValue()) + ',')
            i = 0
            for worker in self.sessionWorkers:
                file.write(worker)
                if i != len(self.sessionWorkers) - 1:
    ...
    def datetime2StrList(dt_list):
        return [(dt.strftime('%Y-%m-%d %H:%M') if dt is not None else str()) for dt in dt_list]
...
class SQLDataProcessor(DataProcessor):
    ...
    def saveData(self):
        ...
        if not self.ensureProcedure():
            return
        self.callMethod(self.procedure.setEpoxyBatch,self.gui.epoxyBatch)                                 # Right Gap [mils]
        self.callMethod(self.procedure.setEpoxyTime,self.gui.getElapsedTime())                              # Min BP/BIR Gap [mils]
        self.callMethod(self.procedure.setDp190,self.gui.DP190Batch) 
        for i in range(len(self.gui.strawList)):
            print("starting now")
            if self.gui.strawList[i] != '':
                straw = self.stripNumber(self.gui.strawList[i])
                print(straw)
