...
to permanently remove " + straws[pos] + " from " + CPAL + " ?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def moveStraw(self, btn):
        ...
        item, okPressed = QInputDialog.getItem(self, "Move Straw","Where would you like to \
move this straw to?", items, 0, False)
        if not okPressed:
            return
        if item == 'This Pallet':
            ...
            if straws[newpos] != 'Empty':
                buttonReply = QMessageBox.question(self, 'Move Error', "Position " + str(newpos) + " is already \
filled by straw " + straws[newpos] + " .", QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
                return
            buttonReply = QMessageBox.question(self, 'Straw Move Confirmation', "Are you sure you \
want to move " + straws[pos] + " from position " + str(pos) + " to \
position " + str(newpos) + " ?", QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
        ...
        if item == 'Another Pallet':
            newpal, okPressed = QInputDialog.getText(self, "Move Straw","Scan the pallet number of the \
pallet the straw will be moved to:", QLineEdit.Normal, "")
            if not okPressed:
                return
            newpal, newlastTask, newstraws, newpassfail = self.getPallet(newpal)
            if newlastTask != lastTask:
                buttonReply = QMessageBox.question(self, 'Move Error', "Pallet " + CPAL + " and \
pallet " + newpal + " are at different stages of production.", QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
                return
            newpos, okPressed = QInputDialog.getInt(self, "Move Straw","New Straw Position on \
" + newpal + ":", pos, 0, 23, 1)
            if not okPressed:
                return
            if newstraws[newpos] != 'Empty':
                buttonReply = QMessageBox.question(self, 'Move Error', "Position " + str(newpos) + " is already \
filled by straw " + newstraws[newpos] + " .", QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
                return
            for palletid in os.listdir(self.palletDirectory):
                for pallet in os.listdir(self.palletDirectory + palletid + '\\'):
