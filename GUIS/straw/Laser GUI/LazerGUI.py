...
#   NOTE: The straw with the largest y value is
#the shortest straw
#
...
dummyPath = os.path.dirname(__file__) + '/../'
...
MAIN_DIR = '\\\\spa-mu2e-network\\Files\\Production_Environment\\GUIS\\straws\\Laser GUI\\' #CSV in CSV_Files, LASERCUT in Original_Lazer_Files
...
class cutMenu(QMainWindow):
    ...
    def __init__(self, webapp=None, parent=None):
        ...
        self.workerDirectory = os.path.dirname(__file__) + '/../../../Data/workers/straw workers/lazer cutting/'
        self.palletDirectory = os.path.dirname(__file__) + '/../../../Data/Pallets/'
        self.laserDirectory = os.path.dirname(__file__) + '/../../../Data/Laser cut data/'
        ...
        self.ui.commentBox.setPlaceholderText('After adding a comment, click on the instruction above to continue.')
    ...
    def keyPressEvent(self, qKeyEvent):
        ...
        if qKeyEvent.key() == Qt.Key_Return and self.scanEntry == 'FIRST':
            self.firstCut()
        if qKeyEvent.key() == Qt.Key_S and self.scanEntry != 'FIR' and self.scanEntry != 'FIRS' and self.scanEntry != 'FINI':
            self.scanEntry = 'S'
            #print('s')
...
# Obtain current temperature and humidy
    def getTempHumid(self):
        ...
        if not found:
            QMessageBox.critical(self, "No Temperature and Humidity Data", "Unable to get current temperature and humidity data. Make sure the temperature and humidity sensors are working, and try again")
            return
    ...
    def saveDummy(self):
        ...
        time.sleep(1)
        #print(MAIN_DIR)
        pyautogui.typewrite(MAIN_DIR + 'Dummy Files\\' + datetime.datetime.now().strftime("%Y-%m-%d_%H_%M_%S"))
        time.sleep(.5)
        pyautogui.press('enter')
        time.sleep(.5)
        print('2')
    ...
    def getPalletNum(self):
        ...
        pallet, ok = QInputDialog().getText(self, 'Pallet Number', 'Please scan the Pallet Number', text = 'CPAL####')
    ...
    def editPallet(self):
        ...
        CPAL, lastTask, straws, passfail = rem.getPallet(self.palletNum) # Run again incase changes were made (straws removed, moved, etc...)
        #After executing
        self.interpretEditPallet(CPAL, lastTask, straws, passfail)
    
    def interpretEditPallet(self, CPAL, lastTask, straws, passfail):

        self.palletInfoVerified = True  # Initially assume True, can only be switched to False

        if lastTask != 'leak':
            self.palletInfoVerified = False
            #self.setStatus(self.palletInfoVerified)
            #Error Message
            QMessageBox.about(self, "Remove Straws", \
        ...
        remove_straws = []
        for i in range(24):
            if (passfail[i] == 'Fail' or passfail[i] == 'Incomplete' and straws[i] != 'Empty'):
        ...
        # If failed straws are present, instruct user to remove them and run editPallet() (which will call this function again)
            ...
            for strawID in remove_straws:
                instructions += '\n' + strawID
            buttonreply = QMessageBox.question(self, "Remove Straws",instructions, QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
    ...
    def firstCut(self):
        ...
        self.loadLaser()
        self.step = 1
        self.ui.instructions.setText('After the laser is done with the first cut, slowly slide the pallet to its second cut position. Once the pallet is in place, scan the SECOND bardcode. The second cut will begin immediately after scanning SECOND.\n\nDo not touch the mouse or keyboard for 60 seconds after pressing Second Cut.')

    def chooseCut(self):
        if self.step != 1:
            reply = QMessageBox.question(self, "First Cut", "Have you done the first cut?", QMessageBox.Yes, QMessageBox.No)
    ...
    # this avoids autogui from accidentally selecting the wrong cut or multiple cuts
        ...
        self.changeSettings()
        self.loadLaser()
        self.ui.instructions.setText('Remove the straw samples from the pallet and insert them into a new film canister. Label the canister with the Pallet # and insert it into the canister storage drawer.\n\nRemove and discard of the scrap ends of the straws and place the pallet on rack A. After this, scan FINISH.')
    ...
    def uploadData(self):
        ...
        for index,straw in enumerate(self.straws):
            ...
            try:
                if straw != "_______":
                    uploader.beginUpload(straw, uploadWorker, t, self.cutTemperature, self.cutHumidity, self.cutLengths[index], 0, self.palletNum)
    ...
    def resetGUI(self):
        ...
        self.ui.palletNumInput.setStyleSheet('background : rgb(255, 255, 255);\nborder-style : solid;\nborder-color: rgb(170, 255, 255);\nborder-width : 2px;')
        self.ui.commentBox.document().setPlainText('')
        self.step = 0
        self.ui.instructions.setText('Once the pallet is properly alligned on the cutting table, scan the pallet number with format CPAL####.\
