...
def checkStrawIntegrity(straw):
    ...
    for thing in StrawPresent.query().filter(StrawPresent.straw == straw.id).all():
        ...
        cpals = (
            ...
            .join(StrawLocation, StrawLocation.id == StrawPosition.location)
...
def load(lpal_file, manual=True):
    ...
    if manual:
        logger.info(
            'Warning: This script will potentially modify the database.  Enter "Y" to continue.'
        )
        response = input()
    ...
    try:
        ...
        logger.error(
            f"Aborting submission for LPAL {lpalNum}, not found in DB.\nException: {e}"
        )
        lpalLoc = None
    ...
    for row in rows:
        ...
        try:
            ...
            logger.error(
                f'Aborting submission for straw {row["Straw"]}, not found in DB and could not be created.\nException: {e}'
            )
            numErrors += 1
            continue
        ...
        try:
            position = (
                StrawPosition.query()
                .filter(StrawPosition.location == lpalLoc.id)
                .filter(StrawPosition.position_number == row["Position"])
                .one_or_none()
            ).id
        except Exception as e:
            logger.error(
                f"Aborting submission for straw {straw.id}, position not found.\nException: {e}"
            )
            numErrors += 1
            continue
        ...
        if inLPAL:
            logger.info(
                f"Aborting submission for straw {straw.id}, LPAL entry already present."
            )
        ...
        if integrity != 0:
            # problem found!
            logger.info(
                f"Aborting submission for straw {straw.id} due to integrity error."
            )
            continue
        ...
        if not inLPAL:
            ...
            logger.info(
                f"Straw: {straw.id}, Position: {position}, Present: {0}, time_in: {row['Timestamp']}, time_out: {panelInTime if inPanel else None}"
            )
            newEntry(
                straw.id,
                position,
                ...
                connection,
                row["Timestamp"],
                (panelInTime if inPanel else None),
            )
        ...
        if integrity != 0:
            # problem found!
            logger.info(
                f"The script may have corrupted the data for straw {straw.id}.  Sowwy! >_<"
            )
            numErrors += 1
