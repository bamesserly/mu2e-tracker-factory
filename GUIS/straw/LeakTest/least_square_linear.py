...
def get_slope(xi, yi, yi_err):
    ...
    num2 = sum_div(mult_lists(xi, yi), sqr_list(yi_err)) * sum(
        sqr_list(inver_list(yi_err))
    )
...
def get_slope_err_zero_intercept(xi, yi, yi_err):
    ...
    s = (
        float(sum(sqr_list(sub_lists(yi, mult_list(xi, a))))) / float(len(xi) - 1)
    ) ** 0.5
...
def jump_check_intercept(time, PPM, err):
    ...
    if (
        abs(intercept_0 - intercept_1) > inter_err0 * Nerr
        or abs(intercept_0 - intercept_1) > inter_err1 * Nerr
    ):
        return 1
    else:
        return 0
