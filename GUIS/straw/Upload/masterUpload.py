...
all_steps = ["makestraw", "resistance", "co2epoxy", "leak", "measurement", "silverepoxy"]
all_stationids = ['make', 'ohms', 'C-O2', 'leak', 'leng', 'silv']

def workerIDError(worker):
    return f"DETAIL:  Key (worker_barcode)=({worker}) is not present in table \"workers\"."
...
class MakeUpload():
    ...
    def beginUpload(self, straw, batch, worker, cpal, source = 'gui'):
        ...
        dataLoader.addRow(self.createRow(data), 'insert')
            ...
            if CPAL in filename:
                ...
                for index, straw in enumerate(straws):
                    if straw in failed_straws:
                        try:
                            self.beginUpload(straw, batches[index], worker, CPAL, 'master')
                            ...
                            if error.message[125:-1] == workerIDError(worker):
                                try:
                                    self.beginUpload(straw, batches[index], master_worker, CPAL, 'master')
                                except UploadFailedError:
                                    errors = open(self.failed_path + "\\" + t + "_masterupload_errors.txt",'a+')
...
class ResistanceUpload():
    def __init__(self, mode):
        self.table = "straw_resistance_measurements"
        self.mode = mode
        self.failed_path = os.path.dirname(__file__)+ '/../../../Data/Failure Data/Upload Failure'
    ...
    def beginUpload(self, straw, worker, temp, humidity, resistance, resistance_type, position, cpal, source = 'gui'):
        if source == 'gui':
        ...
        data = [straw, create_time, worker, workstation, resistance, temp, humidity, test_type, resistance_type]
    ...
    def findDataAndUpload(self, failed_straws, CPAL, worker):
        ...
        test_dict = {"inside-inside" : "ii", "inside-outside" : "io", "outside-inside" : "oi", "outside-outside" : "oo"}
        ...
        for file in os.listdir(self.data_path):
            ...
            if any(in_range):
                path = self.data_path + '\\' + filename
                with open(path, 'r') as f:
                    data_file = csv.reader(f, skipinitialspace=True)
                    for index, row in enumerate(data_file):
                        if index == 0:
                            continue
                        ...
                        if test in tests[i]:
                            ...
                            try:
                                self.beginUpload(straw, worker, temp, humid, resistance, test, position, CPAL, 'master')
                            except UploadFailedError as error:
                                t = datetime.datetime.now().strftime('%Y-%m-%d')
                                original_message = error.message
                                ...
                                if error.message[125:-1] == workerIDError(worker):
                                    try:
                                        self.beginUpload(straw, master_worker, temp, humid, resistance, test, position, CPAL, 'master')
                                    except UploadFailedError:
                                        errors = open(self.failed_path + "\\" + t + "_masterupload_errors.txt",'a+')
                                        errors.write(straw + " " + test + " FAILED UPLOAD\n")
                                        errors.write(original_message)
                                        errors.write("\n")
                                        errors.close()
...
class CO2Upload():
    def __init__(self, mode):
        ...
        self.mode = mode
        self.failed_path = os.path.dirname(__file__)+ '/../../../Data/Failure Data/Upload Failure'
        self.data_path = os.path.dirname(__file__)+ '/../../../Data/CO2 endpiece data'
        ...
        for file in os.listdir(self.data_path):
            ...
            if CPAL in filename:
                ...
                try:
                    ...
                    if error.message[125:-1] == workerIDError(worker):
                        try:
                            self.beginUpload(straw, master_worker, batch, CPAL, 'master')
                        except UploadFailedError:
                            errors = open(self.failed_path + "\\" + t + "_masterupload_errors.txt",'a+')
                            errors.write(straw + " FAILED UPLOAD\n")
                            errors.write(original_message)
...
class LeakUpload():
    def __init__(self, mode):
        ...
        self.mode = mode
        self.failed_path = os.path.dirname(__file__)+ '/../../../Data/Failure Data/Upload Failure'
        self.data_path = os.path.dirname(__file__)+ '/../../../Data/Leak test data/Leak Test Results/Leak Test Results.csv'
    ...
    def createRow(self, data):
        ...
        'worker_barcode' : data[2],
        'workstation_barcode' : str(data[3]),
        'leak_rate' : round(data[4]*(10**5), 2),
        'comments' : str(data[5]),
        'uncertainty' : round(data[6]*(10**5), 2),
        'leak_test_timestamp' : str(data[7])
        }

    def beginUpload(self, straw, worker, leakrate, uncertainty, testtime, chamber, cpal, source = 'gui'):
        if self.mode == "prod":
        ...
        data = [straw, "co2", worker, self.workstation, leakrate, '', uncertainty, testtime]
        ...
        with open(self.data_path, 'r') as f:
            ...
            for index, row in enumerate(all_rows):
                ...
                if straw in failed_straws:
                    test_time = row[1]
                    chamber = int(row[4][7:])
                    leak_rate = float(row[5])
                    uncertainty = float(row[6])

                    try:
                        self.beginUpload(straw, worker, leak_rate, uncertainty, test_time, chamber, CPAL, 'master')
                    except UploadFailedError as error:
                        print(error.message)
                        t = datetime.datetime.now().strftime('%Y-%m-%d')
                        original_message = error.message
                        ...
                        if error.message[125:-1] == workerIDError(worker):
                            try:
                                self.beginUpload(straw, master_worker, leak_rate, uncertainty, test_time, CPAL, 'master')
                            except UploadFailedError:
                                errors = open(self.failed_path + "\\" + t + "_masterupload_errors.txt",'a+')
                                errors.write(straw + " FAILED UPLOAD\n")
                                errors.write(original_message)
                            ...
                            try:
                                self.beginUpload(straw, worker, leak_rate, uncertainty, test_time, CPAL, 'master')
                            except UploadFailedError:
                                if error.message[125:-1] == workerIDError(worker):
                                    try:
                                        self.beginUpload(straw, master_worker, leak_rate, uncertainty, test_time, CPAL, 'master')
...
class MeasurementUpload():
    def __init__(self, mode):
        ...
        self.mode = mode
        self.failed_path = os.path.dirname(__file__)+ '/../../../Data/Failure Data/Upload Failure'
        self.data_path = os.path.dirname(__file__)+ '/../../../Data/Measurement Data'
    ...
    def beginUpload(self, straw, worker, testtime, temp, humidity, nom_length, measured_length, cpal, source = 'gui'):
        if source == 'gui':
            t = datetime.datetime.now().strftime('%Y-%m-%d')
        ...
        data = [straw, worker, self.workstation, round(float(nom_length), 2), round(float(measured_length), 2), temp, humidity, testtime]
    ...
    def findDataAndUpload(self, failed_straws, CPAL, worker):
        ...
        for index, straw in enumerate(straws):
            if straw in failed_straws:
                ...
                try:
                    self.beginUpload(straw, worker, (time + ":00").replace("_", " "), temp, humid, nom_lengths[index], meas_leng, CPAL, 'master')
                except UploadFailedError as error:
                    t = datetime.datetime.now().strftime('%Y-%m-%d')
                    original_message = error.message
                    ...
                    if error.message[125:-1] == workerIDError(worker):
                        try:
                            self.beginUpload(straw, master_worker, (time + ":00").replace("_", " "), temp, humid, nom_lengths[index], meas_leng, CPAL, 'master')
                        except UploadFailedError:
                            errors = open(self.failed_path + "\\" + t + "_masterupload_errors.txt",'a+')
                            errors.write(straw + " FAILED UPLOAD\n")
                            errors.write(original_message)
...
class SilverUpload():
    def __init__(self, mode):
        ...
        self.mode = mode
        self.failed_path = os.path.dirname(__file__)+ '/../../../Data/Failure Data/Upload Failure'
        self.data_path = os.path.dirname(__file__)+ '/../../../Data/Silver epoxy data'
    ...
    def beginUpload(self, straw, worker, epoxy_batch, cpal, source = 'gui'):
        ...
        data = [straw, "termination", worker, self.workstation, epoxy_batch]
                ...
                while line != '':
                    if count % 6 == 0:
                        ...
                        if step == "resistance":
                            ...
                            l = [i for i, v in enumerate(failed_straws) if v[0] == straw]
                            if l == []:
...
def getCPALFromStraws(straws, mode = 'first'):
    path = os.path.dirname(__file__)+ '/../../../Data/Pallets'
    CPAL = dict(zip(straws, ['']*len(straws)))
    straw_copy = straws.copy()
            ...
            for cpal in os.listdir(path + '\\' + cpalid):
                if CPAL in cpal:
                    with open(path + '\\' + cpalid + '\\' + cpal, 'r') as strawFile:
                        ...
                        if mode == 'first':
                            first = row_list[1]
                            straws = list(filter(lambda x : len(x) == 7 and (x[0] == 's' or x[0] == 'S') and (x[1] == 't' or x[1] == 'T') and x[2:].isdigit(), first))
                        elif mode == 'last':
                            last = row_list[-1]
                            straws = list(filter(lambda x : len(x) == 7 and (x[0] == 's' or x[0] == 'S') and (x[1] == 't' or x[1] == 'T') and x[2:].isdigit(), last))
                        else:
                            return []
                    return straws
        return []
...
if __name__ == '__main__': 
    #uploadStepGivenCPAL('wk-bhiltbra01', 'CPAL0077', 'measurement', 'dev')
    #uploadStepGivenStrawsAndCPAL('wk-bhiltbra01',['ST03158','ST03159','ST03160','ST03161','ST03162','ST03163','ST03165','ST03166'],'CPAL0069','makestraw','prod')
    #upload = MakeUpload('prod')
    #upload.beginUpload('ST02870', '110717.B2', 'wk-bhiltbra01', 'CPAL0064')
    #uploadStepGivenCPAL('wk-bhiltbra01', 'CPAL0076', 'leak')
    straws = [f'ST{str(n).zfill(5)}' for n in range(2318, 2321)]
    tests = ['ii','io','oi','oo']
    new_straws = []
    for straw in straws:
        for test in tests:
            new_straws.append((straw, test))
    #print(new_straws)
    uploadStepGivenStraws('wb0001', straws, 'makestraw', 'dev')
    uploadStepGivenStraws('wb0001', new_straws, 'resistance', 'dev')
