...
def arduino_voltage(v5, first_straw):
    ...
    for key, value in sorted(first_straw.items()):
        ...
        while measure_again and repeat_counter <= 20: #Will loop a maximum of 20 times
            ...
            bits_list = ser.readline().decode('utf-8').rstrip().split(',') #bits_list is raw output from arduino
            ...
            while index < len(bits_list):
                ...
                if current_number == 0:
                    ...
                    if new_number != 0: #if new_number is non-zero...
                        nonZero_arduino_measurements[index] = new_number #record new_number
                    if new_number == 0 and current_number == 0: #if new_number is also zero...
                        measure_again = True
...
def calculate_calib(meas_dict, tr1, tr2, straw_nums): #tr = test resistance (resistance of test pallets measured by-hand w/ multimeter (see top)
    calib_dict = {}
    for value in straw_nums:
