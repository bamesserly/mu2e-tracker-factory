...
class CompletionTrack(QtWidgets.QDialog):
    def __init__(self):
        ...
        #Directories
        self.workerDirectory = os.path.dirname(__file__) + '/../../../Data/workers/straw workers/resistance/'
    ...
    def editPallet(self):
        ...
        CPAL, lastTask, straws, passfail = rem.getPallet(self.palletNumber) # Run again incase changes were made (straws removed, moved, etc...)
        #After executing
        self.interpretEditPallet(CPAL, lastTask, straws, passfail)
    ...
    def interpretEditPallet(self, CPAL, lastTask, straws, passfail):
        ...
        if len(remove_straws) == 0:            
            ...
            self.checkForMovedStraws(straws) # Checks if any straws have been moved. If so, transfers data
    ...
    def checkForMovedStraws(self,newStrawIDs):
        ...
        for i1 in range(24):
            ...
            if not self.strawIDs[i1] and (newStrawIDs[i1] in self.strawIDs):
                ...
                self.strawIDs[i1], self.strawIDs[i2] = self.strawIDs[i2], self.strawIDs[i1]

                for j in range(4):
                    self.measurements[i1][j], self.measurements[i2][j] = self.measurements[i2][j], self.measurements[i1][j]
                    self.bools[i1][j], self.bools[i2][j] = self.bools[i2][j], self.bools[i1][j]

                    # Update both positions' displays                   
                    self.meas_input[i1][j].setText(self.resMeasString(self.measurements[i1][j]))
                    self.meas_input[i2][j].setText(self.resMeasString(self.measurements[i2][j]))
    ...
    def collectData(self):
        ...
        if self.palletInfoVerified and self.collectData_counter < self.collectData_limit:
            ...
            if buttonReply == QMessageBox.Ok:
                ...
                for el in self.failed_measurements:
                    ...
                    if meas != 0.0: #meas == 0.0 if no text if entered (user presses x in corner)
                        ...
                        if not any(None in x for x in [i for i,j in zip(self.bools, self.strawIDs) if j]):
                            self.dataRecorded = True
                            self.enableButtons()
                    else:
    ...
    def updatePositionDisplay(self):
        ...
        for i in range(24):
            ...
            if self.strawIDs[i]:
                self.straw_ID_labels[i].setText(self.strawIDs[i])   # Given position box the appropriate straw label
            else: 
            ...
            self.box[i].setEnabled(bool(self.strawIDs[i]))   # (En/Dis)able all elements in box if straw does(n't) exist
    ...
    def getTempHumid(self):
        ...
        filename = ''
        for entry in D:
            if entry.startswith('464B_' + datetime.datetime.now().strftime("%Y-%m-%d")):
                filename = entry
    ...
    def configureSaveData(self):
        ...
        for pos in range(24):
            for i in range(4):
                resistance = self.measurements[pos][i]          #Measurement for respective position and index
                if resistance > 1000:
                ...
                self.saveData[pos][i] = (resistance,pass_fail)  #Saves measurement and pass_fail as a tuple
        ...
        for pos in range(24):
            ...
            for i in range(4):               
                if strawID:
                    ...
                    self.saveFile += ',________,___,____,__,__,____' # CPAL --> pass/fail
    ...
    def save(self):
        ...
        #Prepare file name
        day = datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S_")
        # Get lowest straw id
        i = 0
        ...
        fileName = os.path.dirname(__file__) + '/../../../Data/Resistance Testing/Straw_Resistance_' + day + '_' + first_strawID + '-' + last_strawID + '.csv'
            ...
            for pallet in os.listdir(self.palletDirectory + palletid + '\\'):
                if self.palletNumber + '.csv' == pallet:
                    with open(self.palletDirectory + palletid + '\\' + pallet, 'a') as file:
                        # Record Session Data
                        file.write("\n"+datetime.datetime.now().strftime('%Y-%m-%d_%H:%M') + ',') # Date
    ...
    def uploadData(self):
        ...
        for straw_index, straw in enumerate(self.strawIDs):
            ...
            for measure_index in range(4):
                try:
                    uploader.beginUpload(straw, uploadWorker, temp, humid, self.measurements[straw_index][measure_index], self.meas_type_labels_apprev[measure_index], straw_index + 1, self.palletNumber)
