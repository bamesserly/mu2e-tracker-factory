// jfMeter.ino
//
// Cole Kampa, Aina Zulkifli, Luiz Fernando Andrade, Nick Elscott, Jean-Francois Caron
//
// v2.1
//
...
//
//How to use it:
//Change position with: LEFT(-1) and RIGHT(+1) buttons and UP(48) and DOWN(0) buttons. 
//Switch between wire and straw with: SELECT button.
//Take measurement with: MAIN button(attached to red probe)
...
const Beep low_beep{200, 3, 100};  // Beep for a measurement that is below the acceptable range.
const Beep high_beep{500, 3, 100}; // Beep for a measurement that is above the acceptable range.
const Beep unstable_beep{300 ,3, 100};  //Beep for a measurement with large variance.
...
// NOTE: the numbers are written with decimal places, but to save memory I am using unsigned char
// Unsigned char ranges from 0-255, so this is OK here, but make sure to use a float for calculations.
const unsigned char wires_nominal[max_count] = {149.02,148.57,148.12,147.66,147.20,146.73,146.25,145.77,145.29,144.79,
                                 144.30,143.79,143.28,142.77,142.25,141.72,141.19,140.65,140.10,139.55,
                                 138.99,138.42,137.85,137.27,136.68,136.09,135.49,134.88,134.27,133.64,
                                 133.01,132.38,131.73,131.08,130.42,129.75,129.07,128.38,127.68,126.98,
                                 126.27,125.54,124.81,124.07,123.32,122.56,121.78,121.00,120.21,119.40,
                                 118.59,117.76,116.92,116.07,115.20,114.33,113.44,112.53,111.61,110.68,
                                 109.73,108.77,107.79,106.80,105.79,104.76,103.71,102.64,101.56,100.45,
                                 99.32,98.17,97.00,95.80,94.58,93.33,92.06,90.75,89.41,88.04,86.64,
                                 85.19,83.71,82.19,80.62,79.00,77.32,75.59,73.80,71.93,69.99,67.96,
                                 65.83,63.59,61.21,58.69};
const float wires_range = 0.2; // Must be within 20%.
const float straws_range = 1.0; // Straws must be within 100% of the wire nominal value (ad-hoc)
const float max_variance = 10; // highest acceptble variance of any measurement sequence (ADC counts squared)

// This function returns -1/0/1 if the resistance is below/within/above the valid range 
// for the wire or straw at the given index.
int check_resistance(float resistance, int index, bool wire)
{
  float range;
  if(wire) range = wires_range;
  else     range = straws_range;
  
  float upper = wires_nominal[index]*(1+range);
  float lower = wires_nominal[index]*(1-range);

  if(resistance < lower) return -1;
  else if(resistance > upper) return 1;
  else return 0;
}
...
void setup() {
  ...
  for (int i = 0; i < max_count; i++) {
    wireMeasurements[i] = 0.0/0.0; // Initializing to NaN to differentiate 0.0 ohms from blanks.
...
void loop() 
{
  ...
  if(do_measurement)
  {
    ...
    // check_resistance returns -1/0/+1 for below/within/above valid resistance range.
    //bool valid = !bool(check_resistance(resistance, counter, at_wire));

    if(valid)
    {
      counter = (counter+at_wire) % max_count; //increase counter if we're at_wire, wrap from 95 to 0.  
...
float variance; //global variable used for getting the variance out of the measure_sequence() function.
...
bool manage_measurement() 
{
  ...
  while (i < num_meas) {
    ...
    Serial.print(rawADC);Serial.print(F(", ")); // print every individual measurement to Serial.
  ...
  adc_square_of_means = (adc_total/(1.0*num_meas))*(adc_total/(1.0*num_meas));
  adc_variance = fabs(adc_mean_of_squares - adc_square_of_means);  //variance calculation
  ...
  else if (adc_variance > max_variance) { //for measurements with high variance
    beep(unstable_beep);
