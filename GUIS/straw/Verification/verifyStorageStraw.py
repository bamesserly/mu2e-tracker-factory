...
class verifyStorageStraw:
    def __init__(self):
        ...
        self.verification_file = lambda key: self.storage_directory + self.verification_files[key]
    ...
    def connectRLS(self):
        while not self.rls.connectEncoder():    # Tries to connect. If unsuccessful, runs code below.
                ...
                with open(resistance_dir + "\\" + filename) as f:
                    ...
                    for line in reader:
                        if line["Straw Number"].upper() == strawID.upper():
                            res_data[line[" Measurement Type"].strip().lower()] = float(line[" Resistance(Ohms)"].lower())
    ...
    def checkLeakData(self,strawID):
        ...
        if not data_found:
            ...
            leak_dir = "\\\\MU2E-CART1\\Database Backup\\Leak test data\\Leak Test Results"
            ...
            for file_name in os.listdir(leak_dir):
                if self.strawID in file_name.upper() and ".PDF" in file_name.upper():
                    ...
                    print("Opening pdf...")
                    subprocess.Popen([leak_dir+"\\"+file_name],shell=True) # Opens leak plot pdf
                    ...
                    if self.getYN("Does this plot have reasonable data?"):
                        ...
                        with open("\\\\MU2E-CART1\\Database Backup\\Leak test data\\Leak Test Results\\Leak Test Results.csv","a") as leak_rate_file:
                            leak_rate_file.write("\n")                                                                  # Newline
                            leak_rate_file.write(self.strawID + ",")                                                    # Straw ID
                            leak_rate_file.write(datetime.now().strftime("%m/%d/%Y %H:%M") + ",")                       # Date and Time
                            ...
                            leak_rate_file.write(format(self.leak_rate,  '.2E') + ",")                                  # Leak rate
                            leak_rate_file.write(format(self.leak_error, '.2E') + ",")                                  # Leak rate measurement error
                            leak_rate_file.write("Data location: " + input("Where did you find the leak rate data?"))   # Comments
                    
                    else: # There's something funky with the plot
                        with open("\\\\MU2E-CART1\\Database Backup\\Straw storage\\BenRecalculateLeakTest.txt", 'a') as f:
                            f.write("\n" + self.strawID)
            ...
            if self.verified['leng']:
                ...
                for pos in range(0,95, 2):
                    if (-0.500 <= self.length-self.getDesiredStrawLength(pos) <= 0.500):
                        print("\nThis straw is within tolerance for position " + str(pos) + " ¯\_(*-*)_/¯")
    ...
    def measureWithRuler(self,units="mm"):
        ...
        length = self.rls.convert_in_to_mm((inch_B + prec_B) - (inch_A + prec_A)) + 16.00
        if units == "in":
    ...
    def getDesiredStrawLength(self,pos=None):
        ...
        adjusted_length = nominal_length*(1.0 + (temp_cor*(temp_now-temp_bas)) + (humid_cor*(humid_now-humid_bas)))

        return self.rls.convert_in_to_mm(adjusted_length) + 20.0 # Convert to mm and account for endpieces
    ...
    def verifyStrawID(self,string):
        return (len(string) == 7 and string.upper().startswith("ST") and string[2:].isnumeric())
    ...
    def recordVerification(self,key):
        ...
        fieldnames = ['timestamp','straw','position','workers','ohms','ii','oo','leakRate','leakError','desiredLength','measuredLength','difference','comments']
        data = list()
    ...
    def strawsLeft(self):
        return (len(self.getStraws('storage'))-len(self.getStraws('verified'))-len(self.getStraws('rejected')))
    ...
    def getDataAsDict(self):
        ...
        new_data_dict = {
            ...
            'comments'          :   ''.join([ch for ch in self.comments if ch != ","]).upper() # self.comments, but uppercase and without any ","   
        }
    ...
    def main(self):
        ...
        while not self.verifyWorkerID():
            ...
            if self.verifyWorkerID():
                if not self.getYN("Double-check. Is " + self.workerID + " your worker ID?"):
                    self.workerID = ''
            else:
        ...
        while run_program:
            ...
            while True: # Code-block only gets run once. Breaks as soon as a test fails.

                self.strawID = self.getValidStrawID()

                self.verified['strawID'] = None # This is (1) a valid straw ID, (2) that needs to be verified

                for straw in self.verified_straws:
                    if straw.upper() == self.strawID:
                        print("\nStraw " + self.strawID + " has already been verified! You can put it back in storage.")
                ...
                if self.verified['strawID'] == None:
                    for straw in self.rejected_straws:
                        if straw.upper() == self.strawID:
                            ...
                            self.verified['strawID'] = self.getYN("Do you want to test it again?")
                            break
                ...
                if self.verified['strawID'] == None:
                    print("\nStraw " + self.strawID + " isn't recorded in the storage file.")
                    self.verified['strawID'] = self.getYN("Are you SURE that you entered the correct straw ID?")
                ...
                while self.position == None:
                    pos = input("\nI couldn't find this straw's position. Please enter it: ").strip()
                    while True:
                        if pos.isnumeric() and int(pos) in range(0,95,2):
                            self.verified['position'] = True
                            break
                        pos = input("Invalid postion-- must be an even number from 0-94. Try again.")
                ...
                if self.getYN("Can you see any damage on the straw? "):
                    self.comments += input("Describe the damage. Press ENTER to submit.\n").upper()
            ...
            if all(self.verified.values()):
                ...
                if self.getYN("Was the straw damaged while testing today?"):
                    self.comments += input("Describe the damage. Press ENTER to submit.").upper()
                    self.verified['undamaged'] = False
                else:
                    self.verified['undamaged'] = True
