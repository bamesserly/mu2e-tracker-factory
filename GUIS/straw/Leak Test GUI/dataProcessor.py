...
class TxtDataProcessor(DataProcessor):
    ...
    def saveData(self):
        ...
        with open(self.palletDirectory + self.gui.palletID + '/' + self.gui.palletNumber + '.csv', 'w+') as file:
            
            # Create File Header
            header = 'Time Stamp, Task, 24 Straw Names/Statuses, Workers'
            header += ", ***" + str(self.gui.strawCount) + " straws initially on pallet***"
            header += "\n"
            file.write(header)
