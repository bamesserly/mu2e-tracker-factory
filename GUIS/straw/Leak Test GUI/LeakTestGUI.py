...
### 12/04/18 - Bug fixes
### 2/14/19 - Removed check for inflation step until there is a computer at inflation station
...
from PyQt5.QtWidgets import qApp, QApplication, QLabel, QWidget, QGridLayout, QPushButton,\
     QVBoxLayout, QMainWindow, QDialog, QPushButton, QLineEdit, QMessageBox, QInputDialog
...
class LeakTestStatus(QMainWindow):
    ...
    def __init__(self,COM,baudrate,arduino_input=False):
        ...
        self.COM = ['COM11','COM7','COM9','COM4','COM6','COM10','COM17','COM14','COM15','COM16']
        ...
        self.cpalDirectory = os.path.dirname(__file__) + '/../../../Data/Leak test data/CPALS in Testing/CPALS.txt'
        self.networkDirectory = os.path.dirname(__file__) + '/../../../Data/Leak test data/Leak Test Results/'
        self.workerDirectory = os.path.dirname(__file__) + '/../../../Data/workers/straw workers/leak testing/'
...
#        self.result = self.directory + datetime.now().strftime("%Y-%m-%d_%H%M%S") + '_%s.csv' % self.COM
        ...
        self.StrtData = [self.ui.StartData_1,self.ui.StartData_2,self.ui.StartData_3,self.ui.StartData_4,self.ui.StartData_5,
                         self.ui.StartData_6,self.ui.StartData_7,self.ui.StartData_8,self.ui.StartData_9,self.ui.StartData_10]
        self.StpData = [self.ui.StopData_1,self.ui.StopData_2,self.ui.StopData_3,self.ui.StopData_4,self.ui.StopData_5,
                        self.ui.StopData_6,self.ui.StopData_7,self.ui.StopData_8,self.ui.StopData_9,self.ui.StopData_10]
...
##        self.ui.Log_out.setDisabled(True)
    ...
    def startArduino(self, btn):
        if self.checkCredentials():
            ...
            for x in self.ui.ActionButtons.buttons():
                if (5*ROW - 1) < int(x.objectName().strip('ActionButton')) < (5*(ROW + 1)):
                    x.setEnabled(True)
            for COL in range(self.number_of_chambers):
                self.update_name(ROW,COL)
            thread = threading.Thread(target=self.ReadinBuffer, args=(ROW,), name=("Buffer" + str(ROW)))
            thread.daemon = True                                    # Daemonize thread
            thread.start()
...
##                                              x.setEnabled(True)
                                    ...
                                    if max(PPM[chamber]) > self.max_co2_level and self.passed[chamber] != 'P':
...
##                                        self.Chambers[chamber].setStyleSheet("background-color: rgb(225, 40, 40);")
                                    ...
                                    if len(PPM[chamber]) > 20 and self.leak_rate[chamber] < self.max_leakrate and self.leak_rate_err[chamber] < self.max_leakrate/10:
                                    ...
                                    elif len(PPM[chamber]) > 20 and self.leak_rate[chamber] < self.max_leakrate and eventtime > self.cutOffTime:
                                        #print("Straw in chamber %.0f has Passed, Please remove" % chamber)
                                        straw_status = "Passed leak requirement"
                                        #self.Chambers[chamber].setStyleSheet("background-color: rgb(40, 225, 40);")
                                        self.StrawStatus.emit(chamber, True)
                                        self.passed[chamber] = 'P'
                                    ## Failed straw
                                    elif len(PPM[chamber]) > 20 and self.leak_rate[chamber] > self.max_leakrate and self.leak_rate_err[chamber] < self.max_leakrate/10:
                                        ...
                                        self.StrawStatus.emit(chamber, False)
                                        self.passed[chamber] = 'F'
                                    elif len(PPM[chamber]) > 20 and (self.leak_rate[chamber] - 10*self.leak_rate_err[chamber]) > self.max_leakrate:
                                        #print("FAILURE SHAME DISHONOR: Straw in chamber %.0f has failed, Please remove and reglue ends" % chamber)
                                        straw_status = "Failed leak requirement"
                                        #self.Chambers[chamber].setStyleSheet("background-color: rgb(225, 40, 40);")
                                        self.StrawStatus.emit(chamber, False)
    ...
    def Action(self,btn):
        ...
        if self.chambers_status[ROW][COL][:5] == 'empty':
            ...
            if self.passed[chamber] == 'U':
                ...
                reply = QMessageBox.warning(self, 'Straw Not Finished Leak Testing', msg, QMessageBox.Yes, QMessageBox.No)
                
                if reply == QMessageBox.Yes:
                    thread = threading.Thread(target=self.unloadActionNoSaving, args=(ROW, COL, chamber, btn))
                    thread.daemon = True                                    # Daemonize thread
                    thread.start()
                    return
                else:
            ...
            thread = threading.Thread(target=self.unloadAction, args=(ROW, COL, chamber, btn))
            thread.daemon = True                                    # Daemonize thread
            thread.start()
    ...
    def ChangeStraw(self,chamber):
        ...
        if straw == "empty" : # Empty chamber
                ...
                self.Choosenames[ROW][COL] = straw + '_chamber%.0f_' % chamber + datetime.datetime.now().strftime("%Y_%m_%d")
    ...
    def strawUpload(self, chamber):
        ...
        try:
            uploader.beginUpload(strawname, uploadWorker, leakrate, uncertainty, currenttime, chamber, cpal)
    ...
    def setFailedUpload(self, strawname, message):
        QMessageBox.warning(self, "Upload Error", strawname + " failed upload\n" + message)
    ...
    def signalChecking(self):
        ...
        while True:
            ...
            if ((credentials and not changed) or (not credentials and changed)) and not any(self._running):
...
class StrawSelect(QDialog):
    ...
    def StrawInput(self):
        ...
        if self.straw_load[:2]=='st' or self.straw_load[:2]=='ST' and all(ch.isdigit() for ch in self.straw_load[2:]):
            try:
                ...
                QMessageBox.critical(self, "Testing Error", "Unable to test straw:\nStraw was removed")
            except StrawNotFoundError:
...
class removeStraw(QDialog):
    ...
    def displayPallet(self, CPAL, lastTask, straws, passfail):
        ...
        steps2 = ['Straw Prep','Resistance','CO2 End Pieces','Inflation','Leak Rate','Laser Cut','Length','Silver Epoxy']
