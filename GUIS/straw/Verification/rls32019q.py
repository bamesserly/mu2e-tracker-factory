...
class RLS_E2019Q():
    ...
    def getEncPos(self,con_type=''):
        ...
        try: 
            position = int(''.join([i for i in self.encoder.read(8).decode('utf-8') if i.isnumeric()]))
        except AttributeError:
            print('Encoder not connected, check COM#')
            return
        ...
        if con_type == 'in': return self.convert_to_inches(position)
        elif con_type == 'mm': return self.convert_to_mm(position)
    ...
    def main(self):
        ...
        while True:
            ...
            print("difference: " + format(self.convert_in_to_mm(length-24.002), "5.3f") + " mm")
