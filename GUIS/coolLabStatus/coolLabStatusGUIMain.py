...
class Panel:
    def __init__(self, number, location):
        ...
        self.stepsFinished = [
            ...
            0,
            0,
        ]  # number of finished steps for each procedure
    ...
    def addStep(self, stepPro, timestamp, stepNum):
        self.stepsFinished[
            int(stepPro[3]) - 1
        ] += 1  # add one to number of steps complete for corresponding procedure
...
class labGUI(QMainWindow):
    ...
    def makeSQLTables(self):
        ...
        self.stepsDefTable = sqla.Table(
            "panel_step", self.metadata, autoload=True, autoload_with=self.engine
        )  # step defenitions
        self.stepsDoneTable = sqla.Table(
            "panel_step_execution",
            self.metadata,
            autoload=True,
            autoload_with=self.engine,
        )  # steps completed
        self.sessionsTable = sqla.Table(
            "session", self.metadata, autoload=True, autoload_with=self.engine
        )  # work sessions
        self.workersTable = sqla.Table(
            "worker_login", self.metadata, autoload=True, autoload_with=self.engine
        )  # workers
    ...
    def findStepDefs(self):
        ...
        stepsDefQuery = (
            ...
            .where(self.stepsDefTable.columns.current == 1)  # include current steps
            .where(
                self.stepsDefTable.columns.checkbox == 1  # exclude uncheckable steps
            )
        )

        resultProxy = self.connection.execute(
            stepsDefQuery
        ...
        self.numStepsList = [
            ...
            0,
            0,
        ]  # make list for number of steps for each procedure
        ...
        for i in range(7):  # for i = (0,6); (i+1) = procedure number
            for step in stepsDefResults:  # for each step found earlier,
                if step[1] == f"pan{i+1}":  # if that step is for the (i+1)th procedure
                    self.numStepsList[
                        i
                    ] += 1  # add 1 to the number of steps that procedure has

        for toop in stepsDefResults:  # for each step defenition
            self.steps[toop[0]] = Step(
                toop[0], toop[1], toop[2], toop[3]
            )  # keep it for future referance!  as a step object
    ...
    def findPanels(self):
        ...
        panelsQuery = (
            sqla.select(
                [  # get straw_locations for panels that could be in progress
                    self.panelsTable.columns.id
                ]
            )
            .where(self.panelsTable.columns.number >= 38)  # MN036 is already complete
            .where(
                self.panelsTable.columns.number
                <= 200  # No real panels will have an ID > something in the upper 100's
            )
        )
        resultProxy = self.connection.execute(panelsQuery)  # execute panels query
        panelsResults = (
            resultProxy.fetchall()
        )  # fetch list of straw_location ids in list of tuples: (<location>,)

        refinedPanelsResults = []  # make empty list
        for toop in panelsResults:  # for each tuple in the raw results
            refinedPanelsResults.append(
                toop[0]
            )  # add the actual data to the list we just made

        proceduresQuery = sqla.select(
            [  # find procedures and straw_locations for potentially in progress panels
                self.proceduresTable.columns.straw_location,
                self.proceduresTable.columns.id,
                self.proceduresTable.columns.station,
            ]
        ).where(
            self.proceduresTable.columns.straw_location.in_(
                refinedPanelsResults
            )  # eliminate procedures for already completed panels
        )
        resultProxy = self.connection.execute(
            proceduresQuery
        )  # execute procedures query
        proceduresResults = (
            resultProxy.fetchall()
        )  # fetch list of straw_location ids in list of tuples: (<location>, <procedureID>, <station>)
        ...
        recentPanelQuery = recentPanelQuery.select_from(
            self.panelsTable.join(
                ...
                self.stepsDoneTable,
                self.stepsDoneTable.columns.procedure
                == self.proceduresTable.columns.id,
            )
        )
        ...
        for toop in recentPanels:
            if toop[4] in self.panels:  # if the corresponding panel exists,
                self.panels[toop[4]].addStep(
                    toop[2], toop[1], toop[0]
                )  # add step to panel object
        ...
        self.ui.statusbar.showMessage(
            f"Found panels in {elapsedMiliseconds} miliseconds at {now}", 600000
        )
    ...
    def makeWidget(self, panelObj):
        ...
        recentUpdateLE.setText(
            time.strftime("%a, %m-%d, %H:%M", time.localtime(panelObj.recentUpdate))
        )
        ...
        if self.steps[self.steps[panelObj.currentStep].stepNumber].nextStep is not None:
            tempText = self.steps[
                self.steps[self.steps[panelObj.currentStep].stepNumber].nextStep
            ].stepName.replace("_", " ")
            tempText = tempText.title()
            currentStepLE.setText(tempText)
        else:  # it IS None, we need to get the next procedure
            if (
                self.steps[self.steps[panelObj.currentStep].stepNumber].stepStation
                != "pan7"
            ):
        ...
        proProgBar.setValue(
            (
                panelObj.stepsFinished[panelObj.currentPro - 1]
                / self.numStepsList[panelObj.currentPro - 1]
            )
            * 100
        )
    ...
    def changeColor(self, background_color, text_color):
        ...
        stylesheet = (
            ...
            + " }\n"
            "QPlainTextEdit, QTextEdit, QLabel, QProgressBar { "
    ...
    def refresh(self):
        ...
        for i in reversed(
            range(self.scrollGrid.count())
        ):  # must go backwards, widgets shift position if you start at the beginning
